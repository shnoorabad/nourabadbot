...
def create_pdf_report(records, start_date, end_date):
    ...
    grouped_by_user_date = defaultdict(list)
    for r in records:
        key = (r[0], r[4][:10])  # (name, date)
        grouped_by_user_date[key].append(r)

    grouped_by_user = defaultdict(lambda: defaultdict(list))
    for (name, date), entries in grouped_by_user_date.items():
        grouped_by_user[name][date] = entries

    for name, dates in grouped_by_user.items():
        total_seconds = 0
        c.drawRightString(550, y, reshape(f"نام: {name}"))
        y -= 30

        for date, actions in sorted(dates.items()):
            ...
            c.drawRightString(550, y, reshape(f"{date_shamsi} → {hours} ساعت"))
            y -= 25
    ...
    c.save()def create_pdf_report(records, start_date, end_date):
...
def create_excel_report(records):
    ...
    ws = wb.active
    ws.title = "Attendance"
    ws.append(["نام", "تاریخ", "مدت روزانه (ساعت)"])
    ...
    for name, dates in grouped_by_user.items():
        ...
        for date, actions in sorted(dates.items()):
            ...
            date_shamsi = jdatetime.date.fromgregorian(date=datetime.fromisoformat(date).date()).strftime("%Y/%m/%d")
            hours = round(daily_seconds / 3600, 2)
            ws.append([name, date_shamsi, hours])
